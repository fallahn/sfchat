...
{	
	...
	static const sf::Uint16 DEFAULT_PORT = 1000u;
	...
	//or rename TEAM_ALL
	static const sf::Uint8 TEAM_ALL = 0u;
	static const sf::Uint8 TEAM_MIGHTY_DUCKS = 1u;
	static const sf::Uint8 TEAM_TREDERIA = 2u;
	...
	static const sf::Uint8 ENABLE_ME = 1u;
	static const sf::Uint8 ENABLE_NICK = 1u;
	static const sf::Uint8 ENABLE_TEAM = 1u;
	static const sf::Uint8 ENABLE_CONNECT = 1u;
	static const sf::Uint8 ENABLE_DISCONNECT = 1u;
	static const sf::Uint8 ENABLE_KICK = 1u;
	static const sf::Uint8 ENABLE_PLAY = 1u;
	...
	static const sf::Uint32 UID_KICK = 0xffffff00u; //server is requesting to disconnect the client
	//const sf::Uint32 UID_BAN = 0xffffff01u; //server sending a ban
	static const sf::Uint32 UID_MOTD = 0xffffff02u; //server sending motd
	static const sf::Uint32 UID_SOUND = 0xffffff03u; //server requesting client play sound
	static const sf::Uint32 UID_ACTION = 0xffffff04u; //tells other clients when a client has performed an action
	static const sf::Uint32 UID_TEAM = 0xffffff05u; //let clients know incoming message is a team message
	static const sf::Uint32 UID_SERVER_MESSAGE = 0xffffff06u; //generic server message

	//default colours for output text
	static const sf::Color COLOUR_DEFAULT = sf::Color::Black;
	...
	public:
		...
		const std::deque<TextLine>& GetOutputBuffer(void) const;
	...
	private:
		...
		void m_OutputString(const std::string& str, sf::Color colour = COLOUR_DEFAULT);
		...
		std::string m_ParseCommand(const std::string& str);
		...
		void m_PlaySound(const std::string& sound);
	};
